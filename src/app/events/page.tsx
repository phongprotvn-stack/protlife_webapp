  ...
  return (
    <div className="page-content">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-[24px] font-bold text-[#111] tracking-tight">Sự kiện</h1>
      ...
      {isLoading ? (
        ...
        >
          ...
          <button onClick={loadEvents} className="btn-glass-primary mt-4 px-5 py-2 text-[12px]">
            Thử lại
          </button>
        </motion.div>
        ...
        >
          <div className="w-14 h-14 rounded-full bg-[#007AFF]/5 mx-auto mb-3 flex items-center justify-center">
            <Calendar size={24} className="text-[#007AFF]/30" />
          </div>
          <p className="text-[14px] font-medium text-[#6B7280]">
            {searchQuery || activeFilter ? 'Không tìm thấy kết quả' : 'Chưa có sự kiện nào'}
          </p>
          <p className="text-[12px] text-[#9CA3AF] mt-1">
            {searchQuery || activeFilter ? 'Thử tìm kiếm khác' : 'Bắt đầu ghi lại những sự kiện quan trọng'}
          </p>
        </motion.div>
      ) : (
        <div className="space-y-2.5">
          {filteredEvents.map((event, i) => (
            ...
            >
              <EventCard
                event={event}
                onSelect={isDesktop ? selectEvent : undefined}
              />
            </motion.div>
          ))}
        </div>
