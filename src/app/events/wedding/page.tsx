...
export default function WeddingDashboardPage() {
  ...
  return <div className="page-content min-h-[80vh]">
    ...
    {resolving ? <div className="glass-card p-10 text-center text-[13px] text-[#8E8E93]">Đang chuẩn bị không gian kế hoạch cưới...</div> : noWedding || !eventId ? <EmptyWeddingState onOpen={openWizard} /> : <>
      ...
      <section className="grid gap-4 mt-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(300px,0.75fr)]">
        <article className="glass-card p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="text-[16px] font-bold text-[#111]">Checklist theo timeline</h3><p className="text-[11.5px] text-[#8E8E93] mt-0.5">Bấm vào vòng tròn để hoàn thành công việc.</p></div><button onClick={() => { setTaskError(''); setShowTaskModal(true); }} className="h-[34px] px-3 rounded-[9px] bg-black/[0.05] text-[#4B5563] text-[11px] font-semibold inline-flex items-center justify-center gap-1 hover:bg-black/[0.08] transition-colors"><Plus size={14} /> Thêm việc</button></div><div className="flex gap-1.5 overflow-x-auto mt-4 pb-1">{TIMELINE_FILTERS.map(filter => <button key={filter.id} onClick={() => setTimeline(filter.id)} className={`h-[30px] whitespace-nowrap px-3 rounded-full text-[10.5px] font-semibold transition-all ${timeline === filter.id ? 'bg-[#E6002D] text-white shadow-sm' : 'bg-black/[0.04] text-[#667085] hover:bg-black/[0.07]'}`}>{filter.label}</button>)}</div><div className="mt-4 space-y-2">{tasksLoading ? <p className="py-8 text-center text-[12px] text-[#8E8E93]">Đang tải checklist...</p> : filteredTasks.length === 0 ? <p className="py-8 text-center text-[12px] text-[#8E8E93]">Không có công việc trong mốc thời gian này.</p> : filteredTasks.map(task => <TaskRow key={task.TaskID} task={task} weddingDate={event?.StartDate} onToggle={toggleTask} />)}</div></article>
        <aside className="glass-card p-4 sm:p-5"><div className="flex items-center gap-2"><ReceiptText size={16} className="text-[#E6002D]" /><div><h3 className="text-[15px] font-bold text-[#111]">Phân bổ ngân sách</h3><p className="text-[11px] text-[#8E8E93]">Dự toán {formatVND(totalProjected)} đ</p></div></div><div className="space-y-4 mt-5">{expensesLoading ? <p className="text-[12px] text-[#8E8E93]">Đang tải dòng tiền...</p> : budgetGroups.map(group => <div key={group.id}><div className="flex items-center justify-between gap-2 text-[11px]"><span className="font-medium text-[#5F6368]">{group.label}</span><span className="font-semibold text-[#111]">{formatVND(group.actual)} / {formatVND(group.estimated)} đ</span></div><Progress value={group.percent} color={group.color} /></div>)}</div><Link href={`/events/wedding/guests?event=${eventId}`} className="mt-6 w-full h-[36px] rounded-[9px] bg-[#E6002D]/[0.07] text-[#E6002D] text-[11px] font-semibold flex items-center justify-center gap-1.5 hover:bg-[#E6002D]/10 transition-colors"><UserRoundCheck size={14} /> Mở quản lý khách mời</Link></aside>
      </section>
    </>}
