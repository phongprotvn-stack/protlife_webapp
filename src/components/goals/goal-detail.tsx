...
import { Target, Edit3, Trash2, X } from 'lucide-react';

interface Props { goalId: string | null; onClose: () => void; panelMode?: boolean; }
...
export function GoalDetail({ goalId, onClose, panelMode }: Props) {
    ...
    <div>
      ...
      {confirmDelete && (
        <div className="mb-4 p-4 rounded-[14px] bg-[rgba(230,0,45,0.06)] border border-[rgba(230,0,45,0.12)] text-center">
          ...
          <p className="text-[14px] font-semibold text-[#E6002D] mb-1">Xoá mục tiêu này?</p>
      ...
      <div className="text-center py-8">
        <div className="w-16 h-16 rounded-full bg-[rgba(52,199,89,0.08)] mx-auto mb-3 flex items-center justify-center"><Target size={28} className="text-[#34C759]/40"/></div>
        <p className="text-[14px] font-medium text-[#6B7280]">Mục tiêu chưa có dữ liệu</p>
  ...
  return <Modal open={!!goalId} onClose={onClose} title="" maxWidth="420px">{content}</Modal>;
}
