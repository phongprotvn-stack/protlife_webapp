...
import { Heart, Edit3, Trash2, X } from 'lucide-react';
import { useAppStore } from '@/stores/app-store';

interface Props { memoryId: string | null; onClose: () => void; panelMode?: boolean; }
...
export function MemoryDetail({ memoryId, onClose, panelMode }: Props) {
    ...
    <div>
      ...
      <div className="text-center py-8">
        <div className="w-16 h-16 rounded-full bg-[rgba(255,77,106,0.08)] mx-auto mb-3 flex items-center justify-center">
          <Heart size={28} className="text-[#FF4D6A]/40"/>
        </div>
        <p className="text-[14px] font-medium text-[#6B7280]">Ký ức chưa có dữ liệu</p>
  ...
  return <Modal open={!!memoryId} onClose={onClose} title="" maxWidth="420px">{content}</Modal>;
}
