...
function PrimaryButton({ children, onClick, disabled, icon = true }: {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-full py-[15px] rounded-[15px] border-none text-[15px] font-extrabold text-white cursor-pointer
...
function GhostButton({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="w-full py-[13px] rounded-[13px] border bg-transparent cursor-pointer transition-colors
...
export function WelcomeScreen({ onNext, onSkip }: ScreenProps) {
  return (
    <div className="flex flex-col items-center text-center">
      <motion.div {...fadeUp} className="relative mb-9">
        {/* Emitting glow disk */}
        <div className="w-[130px] h-[130px] rounded-[38px] flex items-center justify-center text-[56px]"
          style={{ background: 'linear-gradient(135deg,#D60032 0%,#FF4B3A 100%)', boxShadow: '0 24px 60px rgba(255,0,45,.45)' }}>
          🌍
        </div>
        <motion.div
          className="absolute inset-0 -z-10 rounded-[38px]"
      ...
      <motion.h1 {...fadeT(0.18)} className="text-[40px] font-black tracking-tight leading-none mb-4">
        PROT <span style={{ color: '#FF4B3A' }}>LIFE</span>
      </motion.h1>
      <motion.p {...fadeT(0.26)} className="text-[17px] leading-relaxed max-w-[300px] mb-2" style={{ color: 'rgba(255,255,255,.78)' }}>
        Bộ Não Thứ Hai
      </motion.p>
      <motion.p {...fadeT(0.32)} className="text-[19px] leading-snug max-w-[300px] mb-10" style={{ color: 'rgba(255,255,255,.5)' }}>
        cho cuộc đời <span className="font-bold text-white">bạn</span>.
      </motion.p>

      <motion.div {...fadeT(0.4)} className="w-full max-w-[300px] space-y-3">
...
/* ────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────── */
const NODES = [
  { key: 'people', label: 'Con người', x: 90, icon: '👤' },
  { key: 'event', label: 'Sự kiện', x: 180, icon: '🎯' },
  { key: 'memory', label: 'Ký ức', x: 270, icon: '💭' },
  { key: 'life', label: 'Cuộc đời', x: 360, icon: '🌱' },
];
...
export function NodeAnimationScreen({ onNext }: ScreenProps) {
  ...
  return (
    <div className="flex flex-col items-center text-center h-full">
      <motion.p {...fadeUp} className="text-[14px] font-bold tracking-[.5px] uppercase mb-6" style={{ color: 'rgba(255,255,255,.5)' }}>
        Mỗi mối quan hệ kể một câu chuyện
      </motion.p>

      <div className="relative w-[300px] h-[300px] mx-auto">
        {/* Horizontal flow line */}
        <div className="absolute top-1/2 left-[40px] right-[40px] h-[2px] -translate-y-1/2" style={{ background: 'linear-gradient(90deg,rgba(255,0,45,.6),transparent)' }} />
          ...
          {NODES.slice(0, active).map((n) => (
            <motion.div key={n.key} className="absolute top-0 bottom-0 flex flex-col items-center justify-center"
              ...
              <div className="flex flex-col items-center gap-1.5">
                <motion.div
                  className="w-[46px] h-[46px] rounded-[14px] flex items-center justify-center text-[22px]"
                ...
                <span className="text-[11px] font-semibold whitespace-nowrap" style={{ color: 'rgba(255,255,255,.6)' }}>{n.label}</span>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
      ...
      <motion.p {...fadeT(0.2)} className="text-[15px] mt-2 font-medium max-w-[280px]" style={{ color: 'rgba(255,255,255,.45)' }}>
...
export function AskNameScreen({ onNext }: ScreenProps) {
  ...
  return (
    <div className="flex flex-col">
      <motion.h2 {...fadeUp} className="text-[26px] font-extrabold mb-1">Chúng mình bắt đầu với</motion.h2>
      <motion.div {...fadeT(0.08)} className="text-[22px] mb-9" style={{ color: '#FF4B3A' }}>💬 Tên của bạn?</motion.div>
      ...
      <motion.div {...fadeT(0.24)} className="w-full max-w-[300px] mx-auto mt-auto">
...
export function AskDobScreen({ onNext, onBack }: ScreenProps & { onBack: () => void }) {
  ...
  const previewLife = useMemo(() => {
    ...
    const chapters = [
      { emoji: '👶', label: 'Chào đời' },
      { emoji: '🎒', label: 'Đi học' },
      { emoji: '🎓', label: 'Đại học' },
      { emoji: '💼', label: 'Sự nghiệp' },
      { emoji: '🚀', label: 'Hiện tại' },
    ];
    ...
    return chapters.map((c, i) => {
      const done = i === 0 ? currentIdx >= 1
        ...
        : i === 3 ? currentIdx >= 5
        : currentIdx >= 5;
      return { emoji: c.emoji, label: c.label, done };
    });
  }, [validDob, stage]);
  ...
  return (
    <div className="flex flex-col">
      ...
      <motion.p {...fadeUp} className="text-[26px] font-extrabold leading-tight mb-9">📅 Ngày sinh của bạn?</motion.p>

      <motion.div {...fadeT(0.12)} className="mb-8">
        <input
          autoFocus
          value={input}
          ...
          className="w-full text-center text-[24px] font-bold py-4 rounded-[16px] bg-white/5 border outline-none tracking-[1px] transition-colors"
      ...
      <AnimatePresence>
        {previewLife && (
          <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} className="mb-9 overflow-hidden">
            <div className="rounded-[18px] p-4" style={{ background: 'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.08)' }}>
                ...
                {previewLife.map((s, i) => (
                  <div key={i} className="relative">
                    ...
                    <div className={`flex items-center justify-between ${s.done?'':'opacity-45'}`}>
                      <span className="text-[13px] font-semibold flex items-center gap-1.5"><span>{s.emoji}</span>{s.label}</span>
      ...
      <div className="w-full max-w-[300px] mx-auto space-y-3 mt-auto">
...
export function AddContactScreen({ onNext, onBack }: ScreenProps & { onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <motion.div {...fadeT(0.22)} className="mt-auto w-full max-w-[300px] mx-auto space-y-3">
        {added ? (
  ...
  return (
    <div className="flex flex-col">
      ...
      <motion.div {...fadeT(0.1)} className="mb-8 flex flex-col items-center gap-3">
        <div className="w-16 h-16 rounded-[20px] flex items-center justify-center text-[30px]" style={{background:'rgba(124,58,237,.16)',border:'1px solid rgba(124,58,237,.4)'}}>
          {initial?.title ? '💭' : '📸'}
        </div>
      ...
      <div className="w-full max-w-[300px] mx-auto space-y-3 mt-auto">
        <PrimaryButton onClick={()=>{setMark(true); setTimeout(()=>onNext({marked:true}),700);}}>💖 Có, đó là Ký ức</PrimaryButton>
  ...
  return (
    <div className="flex flex-col items-center text-center">
      <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:'spring',stiffness:180,damping:12}} className="relative mb-6">
        <div className="w-[120px] h-[120px] rounded-[40px] flex items-center justify-center text-[58px]" style={{background:'linear-gradient(135deg,#D60032,#FF4B3A)',boxShadow:'0 24px 60px rgba(255,0,45,.45)'}}>🎉</div>
        {[...Array(6)].map((_,i)=>(
          <motion.span key={i} className="absolute text-[18px]" style={{left:`${-10+i*22}px`,top:i%2?'-12px':'auto'}}
            initial={{opacity:0,y:0}} animate={{opacity:[0,1,0],y:-28}} transition={{duration:1.4,delay:i*.15}}>{['✨','🎊','💖','🌟','🎈','💫'][i]}</motion.span>
        ))}
      </motion.div>

      <motion.h1 {...fadeUp} className="text-[30px] font-black tracking-tighter mb-2">Chúc mừng!</motion.h1>
      ...
      <div className="w-full max-w-[300px]">
        <PrimaryButton onClick={onComplete}>Vào Dashboard →</PrimaryButton>
